...
struct DarwinianAgent : public Agent{
    ...
    DarwinianAgent (std::vector<Strategy>& strategies, int tot_wins, int i) :
    strategies {std::move(strategies)}, total_wins {tot_wins}, id {i} {};

    DarwinianAgent (int id, int num_strategies, int num_indicies_in_strategy) :
            total_wins {0}, id {id} {
        for (int i = 0; i < num_strategies; ++i) {
    ...
    void agent_add_strategy(int test_result, int num_indicies_in_new_strategy) {
        if(test_result == 1) { strategies.emplace_back(Strategy{0, num_indicies_in_new_strategy}); }
    }
...
/*
    virtual signum sin_get_prediction(const MarketHistory &history) {
...
class Darwinism: public EvolutionStrategy {
    int evolutionary_period;
public:
    Darwinism (int evolutionary_cutoff) : evolutionary_period (evolutionary_cutoff) {}

    fptr test_to_add_strategy(const MarketHistory& history){
        if(history.last_n_results_as_bits(evolutionary_period) == 0){

        }
    }

        std::vector<Agent*> select_next_generation (const MarketHistory& history, AgentPool& agent_pool) override {
