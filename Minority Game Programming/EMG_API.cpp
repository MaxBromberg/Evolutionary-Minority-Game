// api.cpp for Evolutionary Minority Game
...
//MarketInitialization Function Definition
...
//NEEDS OPTIMIZATIONS WITH ALGORITHMS LIBRARY
//Would like to make exp. a template with gen typ def.
double Analysis::expVal(const vector<int>& obv){
    double avg = 0;
...
double Analysis::doubleExpVal(const vector<double>& obv){
    double avg = 0;
    for(int i = 0; i < obv.size(); i++){
        avg+=obv[i];
    }
...
double Analysis::variance(const vector<int>& obv){
    vector<int> sqObv(obv.size()); //implement generate
    double variance;
    double obvExp;
    for(int i = 0; i < sqObv.size(); i++){
        sqObv[i] = obv[i]*obv[i];
    }
    obvExp = Analysis::expVal(obv);
    variance = Analysis::expVal(sqObv) - obvExp*obvExp;
    return variance;
} //variance as I'm accustomed to it

double Analysis::literatureVariance(const vector<int>& obv){
    vector<double> sqObvMinusObvMean(obv.size()); //implement generate
    double variance;
    double obvMean = Analysis::expVal(obv);
    for(int i = 0; i < sqObvMinusObvMean.size(); i++){
        sqObvMinusObvMean[i] = (obv[i]-obvMean)*(obv[i]-obvMean);
    }
    variance = Analysis::doubleExpVal(sqObvMinusObvMean);
    return variance;
}
